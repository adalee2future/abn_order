...
# read data
rda_file <- paste0("cached/abn_daily.", params$rpt_date, ".rda")
...
if ( file.exists(rda_file) ) {
  ...
  abn_daily <- read.csv(csv_file)
  abn_daily <- abn_daily %>%
    mutate(order_date=as.Date(order_date))
  save(abn_daily, file=rda_file)
}
...
abn_7_days_abn1_ydnorm_avg <- data.frame(abn_daily %>%
  ...
  summarize(actualcnt=mean(actualcnt), cnt=mean(cnt), ratio_payed=mean(ratio_payed)))
...
* 30天内均值为 `r percent(abn.30days.summary$mean.dlvr_abnormal.rate)`，标准差为 `r percent(abn.30days.summary$sd.dlvr_abnormal.rate)`
<br><br/>
...
p1 <- ggplot(abn_total_abn_cnt, aes(x=order_date, y=value)) +
  geom_area(alpha = 0.3, position = 'identity', aes(fill = type)) +
  theme(
  ...
  ggtitle(paste("异常或者低质量订单量（率）趋势", date_range)) +
  scale_fill_manual(breaks=c("创建订单量", "总异常订单量", "低质量订单量"),
                    values=c(total_order_color, "#A52A2A", "#6495ED"))

abn_total_abn_ratio <- rbind(
  (abn_overall %>% 
...
p2 <- ggplot(abn_total_abn_ratio, aes(x=order_date,y = ratio)) + geom_line(aes(color=type)) + theme(text = element_text(family = 'SimSun')) + scale_linetype(NULL) + theme(legend.title = element_blank(), text = element_text('SimSun'), legend.position = "bottom", panel.grid.major= element_blank(), panel.grid.minor = element_blank())%+replace% theme(panel.background = element_rect(fill = NA)) + ylim(0, max_y) +  scale_color_manual(breaks=c("订单异常率", "订单低质量率"),values=c("#A52A2A", "#6495ED"))
...
rate100 <- abn_rate %>%
  filter(abn == "abn1-ydnorm" & order_date != today) %>% # today data not correct, so remove it
  mutate(value=ratio_payed*100)

rate10 <- abn_rate %>%
  filter(abn=="abn5") %>% # today data not correct, so remove it
...
abn_scaled_rate <- rbind(rate100, rate10, rate1)
...
levels(abn_rate_135$abn) <- list("订单取消_运单异常"="abn1",
                                 "订单取消_运单正常"="abn1-ydnorm",
                                 "订单取消_无运单"="abn3",
                                 "退单"="abn5")
...
p2 <- ggplot(abn_scaled_rate, aes(x=order_date,y=value)) +
  ...
  scale_linetype(NULL) +
  scale_color_manual(breaks=c("订单取消_运单异常",
                              "订单取消_运单正常*100",
                              "订单取消_无运单",
                              "退单*10"),
                     values=c(abn1_color,
...
levels(abn_scaled_rate$abn) <- list(
                                    "订单正常_运单异常"="abn2",
...
p2 <- ggplot(abn_scaled_rate, aes(x=order_date,y=value)) +
  geom_line(aes(y = value, colour = abn)) +
  scale_linetype(NULL) +
  scale_color_manual(breaks=c("订单正常_运单异常", "订单正常_运单超时"),
                     values=c(abn2_color, abn2_chaoshi_color)) +
  theme(legend.title = element_blank(), text = element_text('SimSun'),
        legend.text=element_text(size=8),
        legend.position = "bottom", panel.grid.major= element_blank(),
        panel.grid.minor = element_blank()) %+replace% 
  theme(panel.background = element_rect(fill = NA),
        plot.margin=unit(c(1,1.2,1.2,1.2),"cm")) + ylim(0, 10)
# combine 2 plots
g1 <- ggplot_gtable(ggplot_build(p1 + ggtitle(paste("低质量订单率变化趋势", date_range))))
g2 <- ggplot_gtable(ggplot_build(p2))
pp <- c(subset(g1$layout, name == "panel", se = t:r))
g <- gtable_add_grob(g1, g2$grobs[[which(g2$layout$name == "panel")]], pp$t, 
                       pp$l, pp$b, pp$l)
...
* 本日订单量 `r prettyNum(abn.shop.cancel.actualcnt.today.spread[["1-2-3"]], big.mark=",")`单，占支付订单量`r round(abn.shop.cancel.ratio.today.spread[["1-2-3"]], 2)`%，比前一天`r decide_up_down(diff.yesterday.123)``r round(abs(diff.yesterday.123), 2)`个百分点，<span style="`r text_color(diff.avg.123, diff.avg.123)`">`r decide_up_down(diff.avg.123, GY_DY)`30天内均值`r round(abs(diff.avg.123), 2)`个百分点(`r round(abs(diff.sd.123), 2)`倍标准差)</span>
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=8}
p2 <- ggplot(abn.shop.cancel.rate, aes(x=order_date,y=ratio_payed/100)) +
  geom_line(aes(y = ratio_payed/100, colour = respb_scene_name)) +
  scale_linetype(NULL) +
...
# combine 2 plots
g1 <- ggplot_gtable(ggplot_build(p1 + ggtitle(paste("订单商户取消率变化趋势", date_range))))
g2 <- ggplot_gtable(ggplot_build(p2))
pp <- c(subset(g1$layout, name == "panel", se = t:r))
g <- gtable_add_grob(g1, g2$grobs[[which(g2$layout$name == "panel")]], pp$t, 
                       pp$l, pp$b, pp$l)
